...
## 최신 후속 상태 (2026-09-16, 아래 초기 이전 기록보다 우선)

1366×768 화면 잘림을 수정해 GitHub와 Vercel에 반영했다. 두 PC 해상도, 입력·정지·재시작·순차 해금·저장 복구·효과 제어를 브라우저에서 검사했다. 세부 결과는 docs/QA-2026-09-16.md를 읽는다.
Vercel은 현재 무로그인 공개이며 GitHub 수정의 자동 반영을 확인했다. 관리 API 403은 남아 있지만 공개 배포를 막고 있지 않다.
다음 사용자 단계는 docs/PLAYTEST.md에 따라 A/B 각 10회 직접 플레이하고 최종 난이도를 결정하는 것이다. qa/의 자동 조작 기록은 사람 플레이 증거가 아니다.
