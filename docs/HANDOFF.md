...
1. 동일 PC·브라우저·주소·스테이지·버전에서 A 10회/B 10회 실제 플레이와 CSV 보관. 중앙값/범위/실패 원인으로 최종 난이도 선택.
2. 실제 OS 창 전환 후 자동 정지와 복귀를 수동 확인.
3. 제출 직전 Vercel URL과 GitHub를 새 시크릿 창에서 확인하고 제출 문안 완성.

기존 PC 경계·입력·실제 10분 자동 실행·저장 복구·효과 검사는 QA-2026-09-16.md에 기록되어 있다. 사용자 직접 플레이와 구분한다. 새 PC 준비와 공통 지침 복원은 workspace-common/HOME-RESUME.md를 따른다.
...
위 내용은 초기 관찰이다. 이후 무로그인 접근과 GitHub 변경 반영이 확인되었으므로 설정을 미완료로 취급하지 않는다. 근거는 QA-2026-09-16.md. 제출 문안의 결과물 주소는 https://skt-aleph-project-2-game.vercel.app/ 이며 이번 PC 이동 정리에서는 재검증하지 않았다.
